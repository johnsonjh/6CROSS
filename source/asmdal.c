...
static const char *err_msg[]
    = { "Invalid opcode mnemonic",
        "Missing opcode mnemonic",
        "Invalid label",
        "Multiply defined label",
        "Bad operand",
        "Bad number",
        "Label not allowed",
        "No operand where one was expected",
        "Bad decimal number",
        "Bad octal number",
        "Statment not within the scope of a TITLE/END block",
        "Multiple TITLE found",
        "Bad title",
        "Bad symbol in operand field",
        "Missing label",
        "Symbol not defined",
        "END statement provided courtesy of the assembler",
        "First half of split DC must be numeric",
        "First half of split DC too big",
        "Second half of split DC too big",
        "Badly placed semicolon",
        "Operand not allowed",
        "Columnation error",
        "Bad value offset",
        "Bad JSYS number",
        "Accumulator symbol must be absolute",
        "Undefined accumulator symbol",
        "Bad accumulator form",
        "Index register symbol must be absolute",
        "Undefined index register symbol",
        "Bad index register form",
        "Device symbol must be absolute",
        "Undefined device symbol",
        "Bad device form",
        "No device found in i/o statement",
        "Undefined value symbol",
        "Bad value form",
        "Accumulator number out of range",
        "Index register number out of range",
...
static void
pad6 (char *d, const char *s, int n) /* copy <=6, blank pad */
{
  int i = 0, m = n < 6 ? n : 6;
...
static void
padn (char *d, int dn, const char *s, int n) /* copy <=dn, pad */
{
  int i = 0, m = n < dn ? n : dn;
...
static int
idx_ch (const char *s, int len, char c, int start)
{
...
static int
char_to_sbin (long *out, const char *s, int len, int base)
{
  int i = 0, sign = 1;
  long v = 0;
  ...
  for (; i < len; i++)
    {
      ...
      if (c == ' ')
        {
          break; /* trailing blank terminates */
        }
...
static void
add_symbol (const char *symbol, int value, int ext_loc, int abs_rel,
            int def_und)
{
  ...
  while (P)
    {
      ...
      if (memcmp (symbol, P->symbol, 6) < 0)
        {
          Q = P;
          P = P->l;
        }
      else if (memcmp (symbol, P->symbol, 6) > 0)
        {
          Q = P;
          P = P->r;
        }
...
static int
look_up (const char *symbol, int *value, int *ext_loc, int *abs_rel,
         int *def_und)
{
  struct node *p = root;
...
static int
change_value (const char *symbol, int value)
{
  struct node *p = root;
 ...
 */
static W
mkword0 (int type, int length, int address)
{
...
static void
prepare_ou_record (W ou_word, int reloc, int loc)
{
  if (count == 0)
    {
      rec_addr = loc;
...
static void
write_externals (struct node *t, int ext_loc)
{
  if (!t)
    {
      return;
...
end_1 (void)
{
  ...
  if (validate_symbol (sym) == 0)
    {
      int v;
      if (look_up (sym, &v, NULL, NULL, NULL) == 0)
        {
          start_address = v;
        }
      else
...
static void
entry_extern_1 (void) /* ENTRY and EXTERN are identical */
{
  if (!is_sp (si, 6))
    {
      ERR (E_CANT_LABEL);
...
equ_1 (void)
{
  ...
  if (char_to_sbin (&v, si + 15, 16, 10))
    { /* operand is a symbol */
      ...
      int vv;
      if (look_up (sym, &vv, NULL, NULL, NULL))
        {
          ERR (E_NO_PREV_SYMB);
          return;
        }
...
oct_1 (void)
{
  ...
  else if (char_to_sbin (&n, si + 15, 16, 8))
    {
      ERR (E_BAD_OCTAL);
    }
...
process_mnemonic (void)
{
  ...
  top = NJSYS - 1;
  bot = 0; /* binary search JSYS table */
  while (bot <= top)
    {
  ...
  static const char *pn[9]
      = { "BLOCK ", "DC    ", "END   ", "ENTRY ", "EQU   ",
          "EXTERN", "OCT   ", "TITLE ", "Z     " };
  for (int i = 0; i < 9; i++)
    {
  ...
  if (J < 13)
    {
      ...
      long vb;
      if (char_to_sbin (&vb, value + J, 13 - J, 10) == 0)
        {
          T->value_bias = (int)vb;
        }
      else
...
static void
check_symbol_or_number (int *out, const char *symbol, int *error_flag,
                        int *reloc)
{
  long n;

  *error_flag = 0;
  if (char_to_sbin (&n, symbol, 6, 10) == 0)
    {
      *out = (int)n;
      return;
    }
  ...
  switch (T->type)
    ...
    case 0: /* NORMAL */
      ...
      w |= ((W)(sb & 017)) << 18;
      {
        ...
        if (T->value_bias != 0 && T->reloc == 2)
          {
            ERR (E_EXPRESS_EXT);
          }
        else
          {
            w |= ((W)((sb + T->value_bias) & M18));
          }
      }
      break;
    ...
    case 1: /* I/O */
      ...
      if (is_sp (T->acc, 6))
        {
          ERR (E_NO_DEV); /* DEV redefines ACC */
        }
...
static void
in_order (struct node *t) /* symbol table listing, sorted */
{
  if (!t)
    {
      return;
...
main (int argc, char **argv)
{
  ...
  if (!src)
    {
      fprintf (stderr, "asmdal: cannot open %s\n", srcname);
      return 2;
    }
  ...
  if (!objname)
    {
      objname = dobj = derive (srcname, ".obj");
    }

  if (!lstname)
    {
      lstname = dlst = derive (srcname, ".lst");
    }

  lst = fopen (lstname, "w");
  if (!lst)
    {
      fprintf (stderr, "asmdal: cannot create %s\n", lstname);
      return 2;
    }
