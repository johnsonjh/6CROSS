/*
 * sim6502 - MOS 6502 emulator
 * SPDX-Lixense-Identifier: MIT-0
 *
...
/*
 * run one instruction;
 * return 0 to keep going, 1 on BRK/halt
 */
...
step (void)
{
  ...
  switch (op)
    ...
    default:
      (void)fprintf (stderr, "sim6502: unimplemented opcode %02X at %04X\n",
                     op, (PC - 1) & 0xFFFF);
      return 1;
    }
...
main (int argc, char * * argv)
{
  ...
  (void)fread (mem + (load & 0xFFFF), 1, 65536 - (load & 0xFFFF), f);
  (void)fclose (f);
