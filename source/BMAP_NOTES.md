...
| `BMAP_DA2.XSI` | 787 | **master op-code table**: one line per mnemonic — `MNEM  VAL(oct)  FLAGS(6 binary = MASK² AR RPL PRFS²)  TYPE(dec 0..45)` |
| `BMAP_SI64.XSI` | 525 | `BMAP_LINKOP`: the bootstrap generator. Reads DA2 (op-codes) into an AVL tree in SPOOL, then expands DA1's `{...}` templates to emit `BMAP_C` (the `%INCLUDE`) and `BMAP_SI62` |
...
| `BMAP_C` | — | **generated** `%INCLUDE` for SI61 (initialised tables); `!DELETE`d at end of build, so not in repo |
...
Build chain (`BMAP_CRU`): `DA2` → LINKOP builds the op-code AVL tree in SPOOL →
DA1 `{OPS}`/`{OPROOT}` holes filled → `BMAP_C` + `BMAP_SI62`. So `BMAP_C` is a
*generated* artifact and isn't in the repo. The op-code table content is all in
`DA2`; the AVL-in-SPOOL packing is just the original's storage mechanism. **The
port replaces this entire bootstrap with a sorted op-code array + binary search.**
...
operation / variable) → `SCANOP` looks the op-code up in the tree → dispatch.
...
| 6 | inst w/o variable field | INST |
...
expansion), `SCANOP` 4580 (op-code field → tree search), `DELSCAN` 1259 /
`NEXTFLD` 3860 (delimiter/field tokeniser; `CURRCH`/`NEXTCH`/`DEL`), `VARSCAN`
...
library rather than reverse-engineering it. The record layout below is from
...
sampled GMAP program against the GE-635/Honeywell-6000 reference (op-code bits
   ...
   - **The named phase-5 list is COMPLETE** (instructions 1-6 + EQU/BOOL/SET,
     ...
     **USE/BLOCK (7)** are now done (phase 6).  The remaining tail:
       * **DEC/OCT float (DONE):** `convert` now computes a host `double` for a
         ...
         two words) — with `nBm` binary-scaled to a fixed integer.  A
       ...
       * the specialized instruction families.  **DONE:** **IO (23)** (`IOFORM`
         ...
         **The NSA family (33-36, DONE):** `flags_scan` ports the `FLAGS`
       ...
       * **macro processor (phase 7, DONE)** -- types MACRO (20), macro call
         ...
         **behavioral C-native** route: each macro's body is kept as plain card
         ...
         **MACRO** (`case_macro_def`): captures body cards up to `ENDM`/`ENDOP`,
         ...
         mis-parsed `#10`); the IF quoted-string compare (`IFE '#3','=0'`) is
         ...
         (`tests/bmapnidrp`).  Still deferred: per-card listing of macro-body
...
6. **Object-unit writer (`XUO$*`) — IN PROGRESS (parts 1-2 done):** the real
   ...
   contents* (per `.original/B$OBJECT_C.txt`) in a deterministic self-framed
   stream — 36-bit words as 5 bytes big-endian, each record a header word
   `TYPE(9)|LEN(9)|ARG(18)` + `LEN`-1 payload words, walked by `LEN` to `END`.
   **The TYPE codes match `B$OBJECT_C`'s `B$RECORDSUBS`** (`HEAD`=0, `DNAM`=1,
   `RNAM`=2, `SECT`=3, `EDEF`=4, `EREF`=5, `SDEF`=6, `SREF`=7, `PROG`=10); the
   keyed file's head-record name (`B$HEADKEY.TEXT`) is folded into the HEAD
   ...
   - **Part 2 (DONE):** relocation directives — `xuo_reloc` emits a **PROG(10)
     ...
     **Bug fixed in passing:** SYMTAB's reference path must clear `REL.F` (SI61
     ...
     them set made an absolute EQU reference look relocatable (spurious RELOC
     + `X` listing mark). Verified by `tests/bmaprel.gmap` →
   ...
   - **Part 4 — debug schema (DONE):** the DELTA-debugger schema, gated by a new
     ...
     **DBGNAM(15)** name plus either an **EXST(13)** statement entry (3-word
     ...
     line) or a **VREBL(14)** variable entry (5-word B$VREBL, for a data/absolute
     symbol whose address type `ATYPE[OPNDTYP]`/section-type is non-zero —
     `ADR.W|ADDRTYP|DATATYP=1|REF`, `LOGSIZ=36|LEVEL=ALIGN=1|OPNDTYP`,
