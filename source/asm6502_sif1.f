...
C
      PASS=0
      OPEN (TMUNIT,FILE='asm6502.tm',FORM='FORMATTED',STATUS='REPLACE')
      OPEN (MCUNIT,FILE='asm6502.mc',FORM='FORMATTED',ACCESS='DIRECT',
     1RECL=256,STATUS='REPLACE')
      CLOSE (MCUNIT,STATUS='KEEP')
      OPEN (MCUNIT,FILE='asm6502.mc',FORM='FORMATTED',ACCESS='DIRECT',
     1RECL=256,STATUS='OLD')
      IUNIT=SIUNIT
      CALL DOPASS
...
C
      PASS=1
      OPEN (TMUNIT,FILE='asm6502.tm',FORM='FORMATTED',
     1STATUS='OLD')
      IUNIT=TMUNIT
      CALL DOPASS
      CLOSE (TMUNIT,STATUS='KEEP')
...
C
      ...
      OPEN (TMUNIT,FILE='asm6502.tm',FORM='FORMATTED',
     1STATUS='OLD')
      IF (IAND(OPTION,OPTXR).NE.0)OPEN (XRUNIT,FILE='asm6502.xr',
     1FORM='UNFORMATTED',RECL=40,ACCESS='DIRECT',STATUS='REPLACE')
      IF (IAND(OPTION,OPTOU).NE.0)OPEN (OUUNIT,FILE=OBJNAM,FORM='FORMATTED',STATUS='REPLACE')
      CALL DOPASS
...
1230  IF (HISYM.EQ.0)GOTO 1330
      DO 1240 I=1,HISYM
         IF (IAND(SYMBOL(3,I),1).EQ.0.AND.SYMBOL(1,I).NE.8724152320)GOTO 1250
1240  CONTINUE
      GOTO 1330
1250  N=3*(LOWID/12)
      IF (N.GT.27)N=27
...
1290  LOBUF(J)=BLANK
      CALL LIST
      J=1
1300  IF (IAND(SYMBOL(3,I),1).NE.0.OR.SYMBOL(1,I).EQ.8724152320)GOTO 1320
      IF (J.LT.N)GOTO 1310
      CALL LIST
      GOTO 1280
...
1340  WRITE (LOUNIT,9020)
      ...
      OPEN (XRUNIT,FILE='asm6502.xr',FORM='UNFORMATTED',RECL=40,
...
1040  ARGCNT=0
      ...
      IF (IUNIT.EQ.SIUNIT.OR.IUNIT.EQ.TMUNIT)WRITE (0,9120)ERRCNT,
     1PASS,LNCNT
      IF (IUNIT.EQ.MCUNIT)WRITE (0,9130)ERRCNT,PASS,LNCNT
      IF (IUNIT.EQ.INUNIT)WRITE (0,9140)ERRCNT,LNCNT,FILENM,FILEAC
      BREAK=0
C
...
1060  CURPOS=1
      CALL NXTCHR(CH,TY)
      IF (CH.EQ.5637144576.OR.CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 2700
C
...
1070  CALL NXTCHR(CH,TY)
      ...
      LEN=LEN+1
      GOTO 1070
1080  IF (LEN.GT.0.AND.SAVCHR.EQ.7784628224)GOTO 1090
      CALL PACK(LABEL,SAVCHR)
C
C     SKIP BLANKS UP TO OPCODE
C
1090  IF (CH.EQ.4294967296)GOTO 1100
      IF ((CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC).AND.LABEL(1).EQ.0)
     1GOTO 2700
      IF (CH.EQ.8187281408)GOTO 1110
...
C     HANDLE = EQUATE
C
1110  CALL PACK(OPCODE,9261023232)
...
1120  CALL PACK(OPCODE,CH)
      ...
      IF (CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 1270
      IF (CH.EQ.4294967296)GOTO 1130
...
C
1140  IF (CH.EQ.5234491392)GOTO 1160
      IF (CH.EQ.4563402752)GOTO 1170
1150  CALL NXTCHR(CH,TY)
      IF (CH.EQ.5905580032)ARGCNT=ARGCNT+1
      IF (CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.4294967296.OR.CH.EQ.KEOC)GOTO 1180
      GOTO 1140
C
C     SCAN FOR CLOSING QUOTE
C
1160  CALL NXTCHR(CH,TY)
      IF (CH.EQ.5234491392)GOTO 1150
...
C
      IF (ARGCNT.NE.1.OR.(CH.NE.4697620480.AND.CH.NE.8187281408))GOTO 1190
      ARGTYP=1
      GOTO 1270
C
C     CHECK FOR INDIRECT OR INDEXED OPERAND
C
1190  IF (CH.NE.5368709120)GOTO 1220
C
C     CHECK FOR PRE-INDEXED BY X ((NN,X))
C
      IF (INPBUF(ARGEND-2).NE.5905580032.OR.INPBUF(ARGEND-1).NE.11811160064.OR.INPBUF(
     1ARGEND).NE.5502926848.OR.ARGCNT.NE.2)GOTO 1200
      ARGTYP=2
      GOTO 1270
C
C     CHECK FOR POST-INDEXED BY Y ((NN),Y)
C
1200  IF (INPBUF(ARGEND-2).NE.5502926848.OR.INPBUF(ARGEND-1).NE.5905580032.OR.INPBUF(
     1ARGEND).NE.11945377792.OR.ARGCNT.NE.2)GOTO 1210
      ARGTYP=3
      GOTO 1270
C
C     CHECK FOR INDIRECT ((PPQQ))
C
1210  IF (INPBUF(ARGEND).NE.5502926848.OR.ARGCNT.NE.1)GOTO 1260
      ARGTYP=4
      GOTO 1270
C
C     CHECK FOR A (A)
C
1220  IF (CH.NE.8724152320.OR.ARGBGN.NE.ARGEND)GOTO 1230
      ARGTYP=5
      GOTO 1270
C
C     CHECK FOR INDEXED BY X (NN,X OR PPQQ,X)
C
1230  IF (INPBUF(ARGEND-1).NE.5905580032.OR.INPBUF(ARGEND).NE.11811160064.OR.ARGCNT.NE.
     12)GOTO 1240
      ARGTYP=6
      GOTO 1270
C
...
C
1240  IF (INPBUF(ARGEND-1).NE.5905580032.OR.INPBUF(ARGEND).NE.11945377792.OR.ARGCNT.NE.
     12)GOTO 1250
      ARGTYP=7
      GOTO 1270
C
...
1490  CURPOS=ARGBGN
      ...
      OBJCNT=2
      IF (PASS.NE.2.OR.RELOC.EQ.0.OR.IAND(OPTION,OPTOU).EQ.0)GOTO 2700
C     NO=1 restored: the original ASM6502_SIF1 dropped it here (a 3-arg call to
C     the 4-arg OBJOUT, leaving BYTE unassociated); every sibling call passes 1.
      CALL OBJOUT(4,LOCCTR+1,1,IAND(RELOC,128)+IAND(RELBAS,127))
      GOTO 2700
C
...
1620  IF (ARGTYP.EQ.0)GOTO 2450
      CURPOS=ARGBGN
      PTR=ARGBGN
1630  IF (PTR.GT.ARGEND.OR.INPBUF(PTR).EQ.5905580032)GOTO 1670
      IF (INPBUF(PTR).EQ.5234491392)GOTO 1650
      IF (INPBUF(PTR).EQ.4563402752)GOTO 1660
1640  PTR=PTR+1
      GOTO 1630
1650  PTR=PTR+1
      IF (INPBUF(PTR).NE.5234491392)GOTO 1650
      GOTO 1640
1660  PTR=PTR+1
      IF (INPBUF(PTR).NE.4563402752)GOTO 1660
      GOTO 1640
...
1830  LOCCTR=IAND(((LOCCTR+VAL-1)/VAL)*VAL,65535)
      RELOC=0
      IF (IAND(OPTION,OPTREL).NE.0)RELOC=1
      RELBAS=0
      IF (LOCCTR.GE.256)RELBAS=1
...
1930  IF (CH.EQ.4294967296.OR.CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 2700
      IF (CH.NE.5905580032)GOTO 2450
      GOTO 1860
C
...
1970  CURPOS=ARGBGN
      CALL NXTCHR(CH,TY)
      PTR=1
      IF (CH.EQ.5234491392)GOTO 1980
      IF (CH.EQ.4563402752)GOTO 2010
      GOTO 2450
1980  CALL NXTCHR(CH,TY)
      IF (CH.EQ.5234491392)GOTO 2000
1990  IF (PTR.GT.80)GOTO 1980
      TEXT(PTR)=CH
      PTR=PTR+1
      GOTO 1980
2000  CALL NXTCHR(CH,TY)
      IF (CH.EQ.5234491392)GOTO 1990
      IF (CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)
     1GOTO 2450
      GOTO 2040
2010  CALL NXTCHR(CH,TY)
      IF (CH.EQ.4563402752)GOTO 2030
2020  IF (PTR.GT.80)GOTO 2010
      TEXT(PTR)=CH
      PTR=PTR+1
      GOTO 2010
2030  CALL NXTCHR(CH,TY)
      IF (CH.EQ.4563402752)GOTO 2020
      IF (CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)
     1GOTO 2450
2040  DO 2050 I=1,80
2050  TITLE(I)=BLANK
...
2140  CALL NXTCHR(CH,TY)
      ...
      CALL PACK(LABEL(1),CH)
      GOTO 2140
2150  IF (CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)
     1GOTO 2450
...
2160  CALL PACK(LABEL(1),BLANK)
      ...
      FILEAC='        '
      I=0
2170  OPEN (INUNIT,FILE=FILENM,STATUS='OLD',ACCESS='SEQUENTIAL',FORM=
     1'FORMATTED',ERR=2180)
      IUSTK(1)=IUNIT
      LNSTK(1)=LNCNT
      GOTO 2700
2180  I=I+1
...
2240  CALL NXTCHR(CH,TY)
      ...
      CALL PACK(GO2LBL(1),CH)
      GOTO 2240
2250  IF (CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)
     1GOTO 2450
      GOTO 2700
C
...
2390  CALL NXTCHR(CH,TY)
      ...
      GOTO 2390
2400  I=J
      IF (CH.EQ.5905580032)GOTO 2380
      IF (CH.EQ.7918845952.OR.CH.EQ.4294967296.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 2420
2410  FLAGMC=0
      GOTO 2450
...
2570  CALL NXTCHR(CH,TY)
      IF (CH.EQ.5234491392)GOTO 2580
      IF (CH.EQ.4563402752)GOTO 2590
      IF (CH.NE.5905580032.AND.CH.NE.4294967296.AND.CH.NE.4429185024.AND.CH.NE.7918845952.AND.CH.NE
     1.KEOC)GOTO 2570
      MCLEN(PTR,I)=CURPOS-MCPTR(PTR,I)-1
      IF (CH.EQ.5905580032)GOTO 2560
      GOTO 2700
2580  CALL NXTCHR(CH,TY)
      IF (CH.EQ.5234491392)GOTO 2570
      IF (CH.NE.KEOC)GOTO 2580
      GOTO 2450
2590  CALL NXTCHR(CH,TY)
      IF (CH.EQ.4563402752)GOTO 2570
