...
static void load(const char *fn)
{
    ...
    while (fgets(line, sizeof line, f)) {
        ...
        if (!((line[7]=='0'&&line[8]=='0') || (line[7]=='1'&&line[8]=='0'))) continue;
        cnt  = hx(line + 1, 2);
        addr = hx(line + 3, 4);
...
static void write_bin(const char *fn)
{
    FILE *f = fopen(fn, "wb");
    if (!f) { perror(fn); exit(1); }
    if (hi >= lo) fwrite(img + lo, 1, hi - lo + 1, f);
    fclose(f);
}

static void write_ihex(const char *fn)
{
    FILE *f = fopen(fn, "w");
    int a;
    if (!f) { perror(fn); exit(1); }
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--ihex")) ihex = 1;
        else if (!strcmp(argv[i], "--bin"))  ihex = 0;
        else in = argv[i];
    }
    if (!in) { fprintf(stderr, "usage: ouconv input.obj [-o out] [--bin|--ihex]\n"); return 1; }
    load(in);
    if (hi < lo) { fprintf(stderr, "ouconv: no data records found\n"); return 1; }
    if (!out) { snprintf(defout, sizeof defout, "%s.%s", in, ihex ? "hex" : "bin"); out = defout; }
    if (ihex) write_ihex(out); else write_bin(out);
