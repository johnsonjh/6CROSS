...
static void
add_sym (const char *nm, int val)
{
  int i;

  for (i = 0; i < nsym; i++)
    {
      if (!memcmp (symname[i], nm, 8))
        {
          symval[i] = val;
          return;
        }
  ...
  while (fgets (line, sizeof (line), f))
    {
      ...
      else if (line[7] == '8' && line[8] == '2')
        { /* external ref */
          int i;
          for (i = 0; i < ndef; i++)
            {
              if (!memcmp (defname[i], line + 9, 16))
                {
                  extmap[addr & 0xFF] = i + 1;
                  break;
                }
            }
        }
      else if (line[7] == '0' && line[8] == '3')
        { /* relocate long */
          for (L = 0; L + 3 <= cnt; L += 3)
            {
              if (nrel < MAXREL)
      ...
      else if (line[7] == '8' && (line[8] == '0' || line[8] == '1'))
        { /* symbol */
          char nm[9];
          int k;
          for (k = 0; k < 8; k++)
            {
              nm[k] = (char)hx (line + 9 + 2 * k, 2);
            }
...
write_rununit (const char *fn)
{
  ...
  if (hi < lo)
    {
      fprintf (stderr, "cp6link: nothing to link\n");
      return;
    }
...
write_schema (const char *fn)
{
  ...
  for (i = 0; i < nsym; i++)
    {
      sum = 0x09 + ((symval[i] >> 8) & 0xFF) + (symval[i] & 0xFF) + 0x80
            + 0x01;
...
main (int argc, char **argv)
{
  ...
  if (nobj == 0)
    {
      fprintf (stderr,
               "usage: cp6link [-s schema] object.obj ... [-o out.ru]\n");
      return 1;
    }
