...
C
      ...
      DATA SYMHED/11140071424,11945377792,10334765056,8858370048,
     1  10603200512,10200547328,4294967296,11542724608,8724152320,
     2  10200547328,11408506880,9261023232,11140071424/
C     UNDHED is packed at runtime in the init block below.
      DATA XRHEAD/
     1  8992587776,11005853696,10603200512,11140071424,11140071424,
     2  6039797760,11005853696,9261023232,9395240960,9261023232,
     3  11005853696,9261023232,10468982784,8992587776,9261023232/
C     XLTAR..XEXTL are packed at runtime in the init block below.
C
...
C
      ...
      CALL ASMINI
      CALL CP6ARG
      CALL S2W('UNDEFINED SYMBOL(S):',5,4,UNDHED)
      CALL S2W('  <-',1,4,XLTAR)
      CALL S2W('  ->',1,4,XRTAR)
      CALL S2W('  - ',1,4,XMINS)
      CALL S2W('  + ',1,4,XPLUS)
      CALL S2W('  * ',1,4,XASTR)
      CALL S2W('EXT ',1,4,XEXT)
      CALL S2W('ENT ',1,4,XENT)
      CALL S2W('UND ',1,4,XUND)
      CALL S2W('(R) ',1,4,XREL)
      CALL S2W('EXTS',1,4,XEXTS)
      CALL S2W('EXTL',1,4,XEXTL)
      PASS=-1
      CALL GETOPT
      HOLDOP=OPTION
      HOLDCL=CLOCK
...
C
      PASS=0
      OPEN(TMUNIT,FILE='asmz80.tm',FORM='FORMATTED',STATUS='REPLACE')
      OPEN(MCUNIT,FILE='asmz80.mc',FORM='FORMATTED',ACCESS='DIRECT',
     1  RECL=256,STATUS='REPLACE')
      CLOSE(MCUNIT,STATUS='KEEP')
      OPEN(MCUNIT,FILE='asmz80.mc',FORM='FORMATTED',ACCESS='DIRECT',
     1  RECL=256,STATUS='OLD')
      IUNIT=SIUNIT
      CALL DOPASS
...
C
      PASS=1
      OPEN(TMUNIT,FILE='asmz80.tm',FORM='FORMATTED',
     1  STATUS='OLD')
      IUNIT=TMUNIT
      CALL DOPASS
      CLOSE(TMUNIT,STATUS='KEEP')
...
C
     ...
     1  OPEN(XRUNIT,FILE='asmz80.xr',FORM='UNFORMATTED',RECL=40,
     2    ACCESS='DIRECT',STATUS='REPLACE')
      IF(IAND(OPTION,OPTOU).NE.0)
     1  OPEN(OUUNIT,FILE=OBJNAM,FORM='FORMATTED',STATUS='REPLACE')
      CALL DOPASS
...
C
 ...
 4810 LOBUF(J)=BLANK
      CALL LIST
      J=1
 4820 IF(IAND(SYMBOL(3,I),1).NE.0.OR.SYMBOL(1,I).EQ.8724152320)GOTO 4840
      IF(J.LT.N)GOTO 4830
      CALL LIST
      GOTO 4800
...
C
 ...
 4901 WRITE(LOUNIT,902)
      ...
      OPEN(XRUNIT,FILE='asmz80.xr',FORM='UNFORMATTED',RECL=40,
...
C
      ...
      CALL S2W(' END;SUPPLIED BY ASM',20,1,ENDCMD)
      CALL S2W('-   ',1,4,XMINS)
      CALL S2W('+   ',1,4,XPLUS)
      CALL S2W('*   ',1,4,XASTR)
      CALL S2W('.   ',1,4,XPERD)
      CALL S2W('/   ',1,4,XSLSH)
      CALL S2W('R   ',1,4,XRCON)
      CALL S2W('X   ',1,4,XXCON)
C
...
C
 2000 ARGCNT=0
      ...
      IF(IUNIT.EQ.SIUNIT.OR.IUNIT.EQ.TMUNIT)WRITE(0,912)ERRCNT,PASS,
     1  LNCNT
      IF(IUNIT.EQ.MCUNIT)WRITE(0,913)ERRCNT,PASS,LNCNT
      IF(IUNIT.EQ.INUNIT)WRITE(0,914)ERRCNT,PASS,LNCNT,FILENM,FILEAC
      BREAK=0
C
...
C
 2020 CURPOS=1
      CALL NXTCHR(CH,TY)
      IF(CH.EQ.5637144576.OR.CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 4200
C
...
C
 ...
 2030 CALL NXTCHR(CH,TY)
      ...
      LEN=LEN+1
      GOTO 2030
 2035 IF(LEN.GT.0.AND.SAVCHR.EQ.7784628224)GOTO 2040
      CALL PACK(LABEL,SAVCHR)
C
C     SKIP BLANKS UP TO OPCODE
C
 2040 IF(CH.EQ.4294967296)GOTO 2050
      IF((CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC).AND.LABEL(1).EQ.0)
     1  GOTO 4200
      IF(CH.EQ.8187281408)GOTO 2055
...
C     HANDLE = OPCODE
C
 2055 CALL PACK(OPCODE,9261023232)
      CALL PACK(OPCODE,10871635968)
      CALL PACK(OPCODE,11408506880)
      GOTO 2070
C
...
C
 2060 CALL PACK(OPCODE,CH)
      ...
      IF(CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 2500
      IF(CH.EQ.4294967296)GOTO 2070
...
C
 2080 IF(CH.EQ.5234491392)GOTO 2100
      IF(CH.EQ.4563402752)GOTO 2110
 2090 CALL NXTCHR(CH,TY)
      IF(CH.EQ.5905580032)ARGCNT=ARGCNT+1
      IF(CH.EQ.5905580032.AND.FCOMMA.EQ.0)FCOMMA=CURPOS-1
      IF(CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.4294967296.OR.
     1  CH.EQ.KEOC)GOTO 2120
      GOTO 2080
C
C     SCAN FOR CLOSING QUOTE
C
 2100 IF(INPBUF(CURPOS-3).EQ.8724152320.AND.INPBUF(CURPOS-2).EQ.9395240960)GOTO 2090
 2105 CALL NXTCHR(CH,TY)
      IF(CH.EQ.5234491392)GOTO 2090
...
C
      ...
      ARG2VL=IAND(ARG2VL-LOCCTR-2,65535)
      IF(ARG2VL.GE.128.AND.ARG2VL.LE.65407)ERRORS=IOR(ERRORS,ERRT)
      OBJECT(2)=IAND(ARG2VL,255)
      ITIME=7
      JTIME=12
...
C
      OBJCNT=2
      OBJECT(1)=24
      ARG1VL=IAND(ARG1VL-LOCCTR-2,65535)
      IF(ARG1VL.GE.128.AND.ARG1VL.LE.65407)ERRORS=IOR(ERRORS,ERRT)
      OBJECT(2)=IAND(ARG1VL,255)
      ITIME=12
...
C
 3020 IF(ARGCNT.EQ.0)GOTO 3630
      CURPOS=ARGBGN
      PTR=ARGBGN
 3030 IF(PTR.GT.ARGEND.OR.INPBUF(PTR).EQ.5905580032)GOTO 3070
      IF(INPBUF(PTR).EQ.5234491392)GOTO 3050
      IF(INPBUF(PTR).EQ.4563402752)GOTO 3060
 3040 PTR=PTR+1
      GOTO 3030
 3050 PTR=PTR+1
      IF(INPBUF(PTR).NE.5234491392)GOTO 3050
      GOTO 3040
 3060 PTR=PTR+1
      IF(INPBUF(PTR).NE.4563402752)GOTO 3060
      GOTO 3040
...
C
 ...
 3215 LOCCTR=IAND(((LOCCTR+VAL-1)/VAL)*VAL,65535)
      RELOC=0
      IF(IAND(OPTION,OPTREL).NE.0)RELOC=1
      RELBAS=0
      IF(LOCCTR.GE.256)RELBAS=1
...
C
 ...
 3310 IF(CH.EQ.4294967296.OR.CH.EQ.7918845952.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 4200
      IF(CH.NE.5905580032)GOTO 3630
      GOTO 3250
C
...
C
 ...
 3345 CURPOS=ARGBGN
      CALL NXTCHR(CH,TY)
      PTR=1
      IF(CH.EQ.5234491392)GOTO 3350
      IF(CH.EQ.4563402752)GOTO 3360
      GOTO 3630
 3350 CALL NXTCHR(CH,TY)
      IF(CH.EQ.5234491392)GOTO 3352
 3351 IF(PTR.GT.80)GOTO 3350
      TEXT(PTR)=CH
      PTR=PTR+1
      GOTO 3350
 3352 CALL NXTCHR(CH,TY)
      IF(CH.EQ.5234491392)GOTO 3351
      IF(CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)GOTO 3630
      GOTO 3370
 3360 CALL NXTCHR(CH,TY)
      IF(CH.EQ.4563402752)GOTO 3362
 3361 IF(PTR.GT.80)GOTO 3360
      TEXT(PTR)=CH
      PTR=PTR+1
      GOTO 3360
 3362 CALL NXTCHR(CH,TY)
      IF(CH.EQ.4563402752)GOTO 3361
      IF(CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)GOTO 3630
 3370 DO 3375 I=1,80
 3375 TITLE(I)=BLANK
      DO 3380 I=1,PTR-1
...
C
 ...
 3430 CALL NXTCHR(CH,TY)
      ...
      CALL PACK(LABEL(1),CH)
      GOTO 3430
 3440 IF(CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)GOTO 3630
      IF(IUNIT.NE.SIUNIT.AND.IUNIT.NE.TMUNIT)GOTO 3630
      DO 3445 I=1,7
 3445 CALL PACK(LABEL(1),BLANK)
      CALL W2S(LABEL,2,4,FILENM)
      FILEAC='        '
      I=0
 3450 OPEN(INUNIT,FILE=FILENM,STATUS='OLD',ACCESS='SEQUENTIAL',FORM=
     1  'FORMATTED',ERR=3460)
      IUSTK(1)=IUNIT
      LNSTK(1)=LNCNT
      GOTO 4200
 3460 I=I+1
      IF(I.GT.8.OR.SRACCT(1,I).EQ.0)GOTO 3470
      CALL W2S(SRACCT(1,I),2,4,FILEAC)
      GOTO 3450
...
C
 ...
 3520 CALL NXTCHR(CH,TY)
      ...
      CALL PACK(GO2LBL(1),CH)
      GOTO 3520
 3530 IF(CH.NE.4294967296.AND.CH.NE.7918845952.AND.CH.NE.4429185024.AND.CH.NE.KEOC)GOTO 3630
      GOTO 4200
C
...
C
 ...
 3615 CALL NXTCHR(CH,TY)
      ...
      GOTO 3615
 3616 I=J
      IF(CH.EQ.5905580032)GOTO 3614
      IF(CH.EQ.7918845952.OR.CH.EQ.4294967296.OR.CH.EQ.4429185024.OR.CH.EQ.KEOC)GOTO 3619
 3618 FLAGMC=0
      GOTO 3630
 3619 IF(PASS.NE.0)GOTO 4200
      CALL WRLINR(MCUNIT,MCREC,256,LOCBUF)
      MCREC=MCREC+1
      GOTO 4200
C
...
C
 ...
 3671 MCHEAD(PTR,I)=INPBUF(PTR)
      CALL RDLINR(MCUNIT,MCREC,256,INPBUF,IERR)
      IF(IERR.NE.0)GOTO 3680
      MCREC=MCREC+1
      CURPOS=1
      ARS=256
      DO 3673 PTR=1,32
 ...
 3677 CALL NXTCHR(CH,TY)
      IF(CH.EQ.5234491392)GOTO 3678
      IF(CH.EQ.4563402752)GOTO 3679
      IF(CH.NE.5905580032.AND.CH.NE.4294967296.AND.CH.NE.4429185024.AND.CH.NE.7918845952.AND.
     1  CH.NE.KEOC)GOTO 3677
      MCLEN(PTR,I)=CURPOS-MCPTR(PTR,I)-1
      IF(CH.EQ.5905580032)GOTO 3676
      GOTO 4200
 3678 CALL NXTCHR(CH,TY)
      IF(CH.EQ.5234491392)GOTO 3677
      IF(CH.NE.KEOC)GOTO 3678
      GOTO 3630
 3679 CALL NXTCHR(CH,TY)
      IF(CH.EQ.4563402752)GOTO 3677
...
C
 4000 IF(PASS.NE.0.OR.FLAGMC.EQ.0)GOTO 4010
      CALL WRLINR(MCUNIT,MCREC,256,INPBUF)
      MCREC=MCREC+1
 4010 IF(OPTYPE(OPIDX).EQ.43)FLAGMC=0
      IF(PASS.NE.0)GOTO 4020
      IF(IUNIT.EQ.SIUNIT)CALL WRLIN(TMUNIT,256,INPBUF)
...
C
  100 CH=INPBUF(ARGB)
      ARGTY=1
      IF(CH.EQ.8858370048)RETURN
      ARGTY=2
      IF(CH.EQ.8992587776)RETURN
      ARGTY=3
      IF(CH.EQ.9126805504)RETURN
      ARGTY=4
      IF(CH.EQ.9261023232)RETURN
      ARGTY=5
      IF(CH.EQ.9663676416)RETURN
      ARGTY=6
      IF(CH.EQ.10200547328)RETURN
      ARGTY=8
      IF(CH.EQ.8724152320)RETURN
      ARGTY=9
      IF(CH.EQ.9797894144)RETURN
      ARGTY=10
      IF(CH.EQ.11005853696)RETURN
      ARGTY=12
      IF(CH.EQ.12079595520)RETURN
      ARGTY=17
      IF(CH.EQ.10737418240)RETURN
      ARGTY=18
      IF(CH.EQ.10334765056)RETURN
      GOTO 800
C
...
C
  200 IF(ARGE-ARGB.GT.1)GOTO 300
      ...
      CH2=INPBUF(ARGE)
      ARGTY=11
      IF(CH1.EQ.10468982784.AND.CH2.EQ.12079595520)RETURN
      ARGTY=13
      IF(CH1.EQ.10468982784.AND.CH2.EQ.8992587776)RETURN
      ARGTY=14
      IF(CH1.EQ.8724152320.AND.CH2.EQ.9395240960)RETURN
      ARGTY=15
      IF(CH1.EQ.10737418240.AND.CH2.EQ.10603200512)RETURN
      ARGTY=16
      IF(CH1.EQ.10737418240.AND.CH2.EQ.9261023232)RETURN
      ARGTY=19
      IF(CH1.EQ.8858370048.AND.CH2.EQ.8992587776)RETURN
      ARGTY=20
      IF(CH1.EQ.9126805504.AND.CH2.EQ.9261023232)RETURN
      ARGTY=21
      IF(CH1.EQ.9663676416.AND.CH2.EQ.10200547328)RETURN
      ARGTY=22
      IF(CH1.EQ.11140071424.AND.CH2.EQ.10737418240)RETURN
      ARGTY=23
      IF(CH1.EQ.9797894144.AND.CH2.EQ.11811160064)RETURN
      ARGTY=24
      IF(CH1.EQ.9797894144.AND.CH2.EQ.11945377792)RETURN
      GOTO 800
C
C     MORE THAN TWO-CHARACTERS IN AF
C
  300 ARGTY=37
      IF(INPBUF(ARGB).EQ.8724152320.AND.INPBUF(ARGB+1).EQ.9395240960.AND.
     1  INPBUF(ARGB+2).EQ.5234491392.AND.ARGE-ARGB.EQ.2)RETURN
      IF(INPBUF(ARGB).NE.5368709120.OR.INPBUF(ARGE).NE.5502926848)GOTO 800
      IF(ARGE-ARGB.GT.2)GOTO 400
      ARGTY=25
      IF(INPBUF(ARGB+1).EQ.8992587776)RETURN
      GOTO 850
C
...
C
  500 IF(ARGE-ARGB.LT.5)GOTO 850
      IF(INPBUF(ARGB+1).NE.9797894144)GOTO 850
      IF(INPBUF(ARGB+3).NE.5771362304.AND.INPBUF(ARGB+3).NE.6039797760)GOTO 850
      ARGTY=0
      IF(INPBUF(ARGB+2).EQ.11811160064)ARGTY=31
      IF(INPBUF(ARGB+2).EQ.11945377792)ARGTY=32
