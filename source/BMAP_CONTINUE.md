...
- Original CP-6 PL/6/etc. sources: `*.XSI` in **`.original/`**. BMAP's are
  ...
  macros), `BMAP_DA2.XSI` (787-line master op-code list), `BMAP_SI64.XSI`
...
- Auto-memory (persists across sessions): `cp6asm-port-state`, `cp6asm-prefs`,
  ...
  (object-unit spec `B$OBJECT_C.txt`, the `XUO$*` object-writer library,
...
3. SI61 does **not** write the object unit itself — it calls `XUO$*` library
   ...
   library rather than reimplementing it. Likewise the routines SI61 declares
...
- **Faithful per-module port**; modernise a routine only when its 9-bit packing
...
- A Horowitz-&-Sahni **AVL** symbol table (BMAP uses the same idea; its names are
  up to 30 chars with `. $ @ _`, vs ASMDAL's 6). BMAP's `TREESRCH` (SI64 lines
...
- Object emission as a self-framed record stream + a byte-walk verifier (the
  python snippet used for ASMDAL) — generalise it for BMAP's record types.
...
(word pack + PC + octal listing + `XLATEV` char-pack); `INST` instruction types
...
-- only single-word numeric `=expr`/`=Oexpr` are done; `FR`/`UFR` forward refs
(the two-pass model resolves ordinary forward references, so this is largely
moot); the aggregate VREBL type variants + B$LBNTRY0 debug header (BMAP needs
only the scalar form); SEGDEF (no directive emits it); the DATE `TTLDAT`/`,1`
path (the TTL-date form; the `DATE` op itself is done); macro refinements (CRSM
auto-generated unique symbols, nested/literal-list IDRP, per-card macro-body
...
nested / literal-list IDRP (`tests/bmapnidrp`).  CRSM and the literal forms also
...
- **`=M` (instruction) and `=V` (VFD) literals.** Other `=` forms are done;
  ...
  packet (a small `gen()` capture mode) rather than computed directly.  `literal()`
...
- **Full listing pagination.** `EJECT`/`TTL`/`TTLS` emit a form feed / a title
  marker line, not true paginated page headers (the listing is not paginated).
- **Object-writer edge cases.** The `FR`/`UFR` forward-reference fixup packet
...
relocation, with `=5` deduped across a macro expansion and direct use),
