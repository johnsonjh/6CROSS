...
check_scan() {                         # $1 = test name (BMAP phase-2 scan trace)
...
for src in "$TDIR"/*.z80; do
    [ -e "$src" ] || continue
    name=$(basename "$src" .z80); cp "$src" "$WORK/"
    ( cd "$WORK" && "$ASMZ80" "$name.z80" >/dev/null 2>&1 )
    check_obj "$name"
done
for src in "$TDIR"/*.s; do
    [ -e "$src" ] || continue
    name=$(basename "$src" .s); cp "$src" "$WORK/"
...
if [ -x "$BMAP" ]; then
    for src in "$TDIR"/*.gmap; do
        ...
        case "$name" in
            bmapscan)                                flag=--scan; chk=check_scan;;
            bmapxref) flag=; chk=check_scan; opt="(LS,XR)";;     # phase 8: cross-reference
            bmaplst)  flag=; chk=check_scan; opt="(LS)";;        # phase 8: listing control
            bmapdbg)                                 flag=-g;     chk=check_obj;;
            bmapobj|bmaprel|bmapdef|bmapref) flag=;   chk=check_obj;;
            bmapsec|bmapflt|bmapio|bmapeis)  flag=;   chk=check_obj;;
            bmapclm|bmapdsc|bmapptr)         flag=;   chk=check_obj;;
            bmapnsa|bmapvec|bmapmcp)         flag=;   chk=check_obj;;
            bmapedc)                                 flag=;   chk=check_obj;;
            bmapdate) flag=; chk=check_obj;  epoch=1614816000;;   # 2021-03-04 UTC
            bmapmac|bmapdup|bmapif|bmapidrp) flag=;   chk=check_obj;;   # phase 7: macros
            bmapcrsm)                                flag=;   chk=check_obj;;   # CRSM auto-symbols
            bmapnidrp)                               flag=;   chk=check_obj;;   # nested/literal-list IDRP
            bmaplit)                                 flag=;   chk=check_obj;;   # phase 8: literals
            bmapxlit)                                flag=;   chk=check_obj;;   # extended literals
            bmapsyn)                                 flag=;   chk=check_obj;;   # phase 8: OPSYN
            bmapall)                                 flag=;   chk=check_obj;;   # phase 9: integration
            *)                                       flag=;       chk=check_scan;;
        esac
    ...
    if [ -f "$ROOT/.original/BMAP_SIG.XSI" ]; then
        ...
        ( cd "$WORK" && "$BMAP" bmapsig.gmap >/dev/null 2>&1 ); rc=$?
        if [ "$rc" -eq 0 ]; then
            check_obj bmapsig
        else
